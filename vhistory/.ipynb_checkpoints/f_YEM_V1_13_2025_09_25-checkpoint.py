...
def DBPG_tdt(D_t, B_t, P_t, G_t, phi_BD, phi_GD, phi_DP, phi_DB, rho_B, phi_PG, rho_G, P_c, G_c, Z_e_t, Z_d_t):
  ...
  G_tdt = G_t + hh(phi_PG * P_t, rho_G * G_t) * 13.33 - phi_GD * G_t - G_c
...
class YellowstoneEcosystemModel:
    ...
    def __init__(self, param_file):
        ...
        # self.bbeta_dh0_P[0] *= UPSCALE
        # self.bbeta_dh0_P[1] *= UPSCALE
        
        self.bbbeta_dp1[0][2] *= UPSCALE
        self.bbbeta_dp1[1][2] *= UPSCALE
        self.bbbeta_dp2[0][2] /= UPSCALE
        self.bbbeta_dp2[1][2] /= UPSCALE

        self.bbeta_sn1[0] *= UPSCALE
        self.bbeta_sn1[1] *= UPSCALE
        self.bbeta_sn2[0] /= UPSCALE
        self.bbeta_sn2[1] /= UPSCALE

        self.ggamma_1[0] *= UPSCALE
        self.ggamma_1[1] *= UPSCALE
        self.ssigma_s[0] *= UPSCALE
        self.ssigma_s[1] *= UPSCALE

        self.bbeta_b1[0] *= UPSCALE
        self.bbeta_b1[1] *= UPSCALE
        self.bbeta_b2[0] /= UPSCALE
        self.bbeta_b2[1] /= UPSCALE

        self.mmu_o[0] *= UPSCALE
        self.mmu_o[1] *= UPSCALE
        self.ssigma_o[0] *= UPSCALE
        self.ssigma_o[1] *= UPSCALE

        # self.ddelta[0] *= UPSCALE ** 0.75
        # self.ddelta[1] *= UPSCALE ** 0.75
        """
    ...
    def simulate(
        ...
        for t in range(1, t_max+1):
            ...
            Pt_d = Gt_d_0(Pt, self.beta_sn0_P, NNt, self.bbeta_dh0_P)
            Pt_dn = Gt_dn_0(Pt, self.beta_sn0_P)
            PPt_dh = GGt_dh_0(Pt, self.beta_sn0_P, NNt, self.bbeta_dh0_P)  # shape: (n_species,)
            Gt_d = Gt_d_0(Gt, self.beta_sn0_G, NNt, self.bbeta_dh0_G)
            Gt_dn = Gt_dn_0(Gt, self.beta_sn0_G)
            GGt_dh = GGt_dh_0(Gt, self.beta_sn0_G, NNt, self.bbeta_dh0_G)  # shape: (n_species,)
            Pt_dh = PPt_dh.sum()
            Gt_dh = GGt_dh.sum()
            ...
            Dt, Bt, Pt, Gt = DBPG_tdt(
                ...
                self.rho_B, self.phi_PG, self.rho_G,
