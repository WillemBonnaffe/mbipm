...
class YellowstoneEcosystemModel:
    ...
    def save_params(self, output_file):
        """
        Save all model parameters to a YAML file based on the CURRENT attribute values.
        Reassignment of attributes (e.g., self.phi_PG = torch.tensor(...)) is respected.
        """
        data = {}

        # Iterate in original YAML order if available
        keys = getattr(self, "_param_keys", list(self.params.keys()))
        for key in keys:
            # Prefer the live attribute value; fall back to dict if missing
            value = getattr(self, key, self.params.get(key))

            if isinstance(value, torch.Tensor):
                t = value.detach()
                if t.ndim == 0:  # scalar tensor
                    data[key] = float(t.item())
                else:            # 1D/2D tensor
                    data[key] = t.cpu().tolist()
            else:
                # If someone replaced a tensor with a plain Python type/list
                data[key] = value

        # Write to YAML (preserve insertion order)
    ...
    def save_params(self, output_file):
        ...
        # Convert all parameter tensors back to Python types
        for key, tensor in self.params.items():
            if isinstance(tensor, torch.Tensor):
                if tensor.ndim == 0:  # scalar
                    data[key] = tensor.item()
                else:  # 1D or 2D list
                    data[key] = tensor.tolist()
            else:
                # In case any parameter is not a tensor (unexpected)
                data[key] = tensor
